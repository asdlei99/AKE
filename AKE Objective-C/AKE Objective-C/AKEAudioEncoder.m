...
// @see: https://github.com/chrisballinger/FFmpeg-iOS-Encoder/blob/master/FFmpegEncoder/AACEncoder.m
@interface AKEAudioEncoder ()
...
@implementation AKEAudioEncoder
...
- (id) init {
    if (self = [super init]) {
        _encodeQueue = dispatch_queue_create("ake.aac.encode.queue", DISPATCH_QUEUE_SERIAL);
...
- (void) setupEncoderFromSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    AKEData *audioSH = [AKEData new];
    [audioSH write_1byte:0xaf];
    [audioSH write_1byte:0];
    uint8_t tmp = (kMPEG4Object_AAC_LC << 3) | ((4 & 0xe) >> 1);
    [audioSH write_1byte:tmp];
    tmp =  ((4 & 0x01) << 7) | (1 << 3);
    [audioSH write_1byte:tmp];

    AKEAudioPacket *packet = [AKEAudioPacket new];
...
{
    AKEAudioEncoder *encoder = (__bridge AKEAudioEncoder *)(inUserData);
...
- (void)encode:(CMSampleBufferRef)sampleBuffer{
    ...
    dispatch_async(_encodeQueue, ^{
        ...
        outAudioBufferList.mBuffers[0].mDataByteSize = self->_aacBufferSize;
        ...
        if (status == 0) {
            ...
            AKEAudioPacket *packet = [AKEAudioPacket new];
            packet.hasEncoded = YES;
            packet.dts = [[AKETimestampManager sharedInstance] audioTimestamp];
            packet.data = [AKEData new];
            [packet.data write_1byte:0xaf];
            [packet.data write_1byte:0x01];
            [packet.data write_data:rawAAC];
            [[AKEAVQueue sharedInstance] enqueue:packet];
        } else {
            error = [NSError errorWithDomain:NSOSStatusErrorDomain code:status userInfo:nil];
        }
